...
function _mailchimp_civix_civicrm_xmlMenu(&$files) {
  foreach (_mailchimp_civix_glob(__DIR__ . '/xml/Menu/*.xml') as $file) {
    $files[] = $file;
  }
}
 ...
 */
function _mailchimp_civix_civicrm_enable() {
  _mailchimp_civix_civicrm_config();
  if ($upgrader = _mailchimp_civix_upgrader()) {
 ...
 */
function _mailchimp_civix_civicrm_disable() {
  _mailchimp_civix_civicrm_config();
  if ($upgrader = _mailchimp_civix_upgrader()) {
 ...
 */
function _mailchimp_civix_civicrm_upgrade($op, CRM_Queue_Queue $queue = NULL) {
  if ($upgrader = _mailchimp_civix_upgrader()) {
 ...
 * @return array(string)
 */
function _mailchimp_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_mailchimp_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
 ...
 */
function _mailchimp_civix_civicrm_managed(&$entities) {
  $mgdFiles = _mailchimp_civix_find_files(__DIR__, '*.mgd.php');
 ...
 */
function _mailchimp_civix_glob($pattern) {
  $result = glob($pattern);
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _mailchimp_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
